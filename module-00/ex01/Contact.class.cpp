...
/*   Updated: 2023/04/23 13:49:51 by mbouthai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Contact::printContact(int index) const
{
	...
	std::cout << "|" << std::setw(WIDTH) << ((this->_firstName.length() > WIDTH) ? truncate(this->_firstName,
				WIDTH) : this->_firstName);

	std::cout << "|" << std::setw(WIDTH) << ((this->_lastName.length() > WIDTH) ? truncate(this->_lastName,
				WIDTH) : this->_lastName);

	std::cout << "|" << std::setw(WIDTH) << ((this->_nickname.length() > WIDTH) ? truncate(this->_nickname,
				WIDTH) : this->_nickname);
...
{
	return (!contact._firstName.empty() && !contact._lastName.empty()
		&& !contact._nickname.empty() && !contact._phoneNumber.empty()
